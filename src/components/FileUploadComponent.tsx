...
const FileUploadComponent: React.FC<FileUploadComponentProps> = ({
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      {/* File upload form */}
      <form
        className={`relative w-full h-60 flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-6 transition-all duration-300 ease-in-out ${
          dragActive 
      ...
      >
        ...
        <div className="text-center">
          {isUploading ? (
            ...
            <>
              <p className={`text-lg ${error ? 'text-error-700' : 'text-gray-700'}`}>
                {error || 'Drag and drop your logo file here'}
              </p>
        ...
        {warnings.length > 0 && (
          <div className="mt-4 w-full">
            {warnings.map((warning, index) => (
              <div key={index} className="flex items-center bg-warning-50 border border-warning-100 text-warning-700 px-4 py-2 rounded-md text-sm mb-2">
                <svg className="w-5 h-5 mr-2 text-warning-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="mt-4 text-center md:hidden">
        <p className="text-xs text-gray-500">
          Tap the upload area to browse files on your device
        </p>
      </div>
    </div>
  );
